...
use crate::{
    actuate_enums::PresetBrowserEntry, release_downloader::ReleaseDownloader,
    CustomWidgets::ComboBoxParam,
};
#[allow(unused_imports)]
use crate::{
    actuate_enums::{
        AMFilterRouting, FilterAlgorithms, LFOSelect, ModulationDestination, ModulationSource,
        PresetType, UIBottomSelection,
    },
    actuate_structs::ActuatePresetV131,
    audio_module::{AudioModule, AudioModuleType},
    Actuate, ActuateParams,
    CustomWidgets::{
        slim_checkbox, toggle_switch,
        ui_knob::{self, KnobLayout},
        BeizerButton::{self, ButtonLayout},
        BoolButton, CustomParamSlider,
        CustomVerticalSlider::ParamSlider as VerticalParamSlider,
    },
    A_BACKGROUND_COLOR_TOP, DARKER_GREY_UI_COLOR, DARKEST_BOTTOM_UI_COLOR, DARK_GREY_UI_COLOR,
    FONT, FONT_COLOR, HEIGHT, LIGHTER_GREY_UI_COLOR, MEDIUM_GREY_UI_COLOR, SMALLER_FONT,
    TEAL_GREEN, WIDTH, YELLOW_MUSTARD,
};
use egui::{Color32, CornerRadius, Margin, Pos2, Rect, RichText, ScrollArea, Vec2};
use nice_plug::{context::gui::AsyncExecutor, editor::Editor, nice_log};
    ...
    if binding.is_some() && dir_files_map.lock().unwrap().is_empty() {
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if path.is_dir() && entry.depth() == 1 {
                ...
                str_files_map.lock().unwrap().insert(
            ...
            // If it's a file inside a level 1 directory, add it to the corresponding directory
                ...
                if let Some(parent_dir) = path.parent() {
                    ...
                    if let Some(files) = str_files_map
                        .lock()
                        .unwrap()
                        .get_mut(parent_dir.file_name().unwrap().to_str().unwrap())
                    {
                        files.push(path.to_path_buf());
                    }
...
pub(crate) fn make_actuate_gui(
    instance: &mut Actuate,
    _async_executor: AsyncExecutor<Actuate>,
    ...
    let lite_db: Arc<RwLock<HashMap<String, HashMap<String, PresetBrowserEntry>>>> =
        instance.preset_browser_lite_db.clone();
    ...
    *bank_current_value.write().unwrap() = "Default".to_string();
    // Open
    let dialog_main_open: Arc<Mutex<FileDialog>> =
        Arc::new(Mutex::new(if (default_dir).exists() {
            FileDialog::new(default_dir.clone(), DialogMode::OpenPreset)
        } else {
            FileDialog::new(home_dir.clone(), DialogMode::OpenPreset)
        }));
    // Save
    let dialog_main_save: Arc<Mutex<FileDialog>> =
        Arc::new(Mutex::new(if (default_dir).exists() {
    ...
    // Export Last Sound
    let dialog_export_last_sound: Arc<Mutex<FileDialog>> =
        Arc::new(Mutex::new(if (default_dir).exists() {
    ...
    // Open Samples
    let dialog_main_open_samples: Arc<Mutex<FileDialog>> =
        Arc::new(Mutex::new(if (default_dir).exists() {
    ...
    create_egui_editor(
        ...
        instance.initial_gui_state.take().unwrap(),
        Default::default(),
        |egui_ctx, _queue, _gui_state| {},
        move |ui, setter, _queue, gui_state| {
            ResizableWindow::new("res-wind")
                    .min_size(Vec2::new(WIDTH as f32, HEIGHT as f32))
...
For constant FM, turn Sustain to 100% and A,D,R to 0%".to_string());
                                                ...
                                                LFOSelect::INFO => {
                                                    ui.horizontal(|ui| {
                                                        ui.add(
                                                            egui::TextEdit::singleline(&mut *params.preset_name_p.lock().unwrap())
                                                                .interactive(true)
