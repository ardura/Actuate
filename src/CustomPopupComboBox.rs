...
fn show_tooltip_at_avoid_dyn<'c, R>(
    ...
    // Rust-analyzer bug here again where frame_state is private?
    //let stored = ctx.frame_state().tooltip_rect;
    let stored = Some(ctx.used_rect());
    //let stored = ctx.memory;
    ...
    /*
    ctx.frame_state().tooltip_rect = Some(frame_state::TooltipRect {
        id,
