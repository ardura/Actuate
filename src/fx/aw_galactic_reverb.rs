...
const DELAYS: [usize; 12] = [
    3407, 1823, 859, 331, 4801, 2909, 1153, 461, 7607, 4217, 2269, 1597,
];
// Missed this originally          I     J     K    L    A     B     C     D    E     F     G     H
const DELAY_SIZING: [usize; 12] = [
...
impl GalacticReverb {
    ...
    pub fn process(&mut self, input_l: f32, input_r: f32) -> (f32, f32) {
        ...
        self.countM += 1;
        self.countM = if self.countM > DELAY_M {
            0
        } else {
            self.countM
        };
        ...
        let mut interpolate_ML = self.aML
            [(workingML - if workingML > DELAY_M { DELAY_M + 1 } else { 0 }) as usize]
            * (1.0 - (offsetML.floor() - offsetML));
        interpolate_ML += self.aML[(workingML + 1
            - if workingML + 1 > DELAY_M {
                DELAY_M + 1
            } else {
                0
            }) as usize]
            * (offsetML - offsetML.floor());
        let mut interpolate_MR = self.aMR
            [(workingMR - if workingMR > DELAY_M { DELAY_M + 1 } else { 0 }) as usize]
            * (1.0 - (offsetMR.floor() - offsetMR));
        interpolate_MR += self.aMR[(workingMR + 1
            - if workingMR + 1 > DELAY_M {
                DELAY_M + 1
            } else {
                0
            }) as usize]
            * (offsetMR - offsetMR.floor());
        ...
        self.cycle = if self.cycle > (cycle_end - 1) {
            cycle_end - 1
        } else {
            self.cycle
        };
        ...
        if self.cycle == cycle_end {
            ...
            self.countI += 1;
            if self.countI > self.delay_bank[0] {
                self.countI = 0;
            }
            self.countJ += 1;
            if self.countJ > self.delay_bank[1] {
                self.countJ = 0;
            }
            self.countK += 1;
            if self.countK > self.delay_bank[2] {
                self.countK = 0;
            }
            self.countL += 1;
            if self.countL > self.delay_bank[3] {
                self.countL = 0;
            }

            let outIL = self.arr_l.a_i[self.countI
                - if self.countI > self.delay_bank[0] {
                    self.delay_bank[0]
                } else {
                    0
                }];
            let outJL = self.arr_l.a_j[self.countJ
                - if self.countJ > self.delay_bank[1] {
                    self.delay_bank[1]
                } else {
                    0
                }];
            let outKL = self.arr_l.a_k[self.countK
                - if self.countK > self.delay_bank[2] {
                    self.delay_bank[2]
                } else {
                    0
                }];
            let outLL = self.arr_l.a_l[self.countL
                - if self.countL > self.delay_bank[3] {
                    self.delay_bank[3]
                } else {
                    0
                }];
            let outIR = self.arr_r.a_i[self.countI
                - if self.countI > self.delay_bank[0] {
                    self.delay_bank[0]
                } else {
                    0
                }];
            let outJR = self.arr_r.a_j[self.countJ
                - if self.countJ > self.delay_bank[1] {
                    self.delay_bank[1]
                } else {
                    0
                }];
            let outKR = self.arr_r.a_k[self.countK
                - if self.countK > self.delay_bank[2] {
                    self.delay_bank[2]
                } else {
                    0
                }];
            let outLR = self.arr_r.a_l[self.countL
                - if self.countL > self.delay_bank[3] {
                    self.delay_bank[3]
                } else {
                    0
            ...
            //const DELAYS: [usize; 12] = [3407, 1823, 859, 331, 4801, 2909, 1153, 461, 7607, 4217, 2269, 1597];
            self.countA += 1;
            if self.countA > self.delay_bank[4] {
                self.countA = 0;
            }
            self.countB += 1;
            if self.countB > self.delay_bank[5] {
                self.countB = 0;
            }
            self.countC += 1;
            if self.countC > self.delay_bank[6] {
                self.countC = 0;
            }
            self.countD += 1;
            if self.countD > self.delay_bank[7] {
                self.countD = 0;
            }

            let outAL = self.arr_l.a_a[self.countA
                - if self.countA > self.delay_bank[4] {
                    self.delay_bank[4]
                } else {
                    0
                }];
            let outBL = self.arr_l.a_b[self.countB
                - if self.countB > self.delay_bank[5] {
                    self.delay_bank[5]
                } else {
                    0
                }];
            let outCL = self.arr_l.a_c[self.countC
                - if self.countC > self.delay_bank[6] {
                    self.delay_bank[6]
                } else {
                    0
                }];
            let outDL = self.arr_l.a_d[self.countD
                - if self.countD > self.delay_bank[7] {
                    self.delay_bank[7]
                } else {
                    0
                }];
            let outAR = self.arr_r.a_a[self.countA
                - if self.countA > self.delay_bank[4] {
                    self.delay_bank[4]
                } else {
                    0
                }];
            let outBR = self.arr_r.a_b[self.countB
                - if self.countB > self.delay_bank[5] {
                    self.delay_bank[5]
                } else {
                    0
                }];
            let outCR = self.arr_r.a_c[self.countC
                - if self.countC > self.delay_bank[6] {
                    self.delay_bank[6]
                } else {
                    0
                }];
            let outDR = self.arr_r.a_d[self.countD
                - if self.countD > self.delay_bank[7] {
                    self.delay_bank[7]
                } else {
                    0
            ...
            //const DELAYS: [usize; 12] = [3407, 1823, 859, 331, 4801, 2909, 1153, 461, 7607, 4217, 2269, 1597];
            self.countE += 1;
            if self.countE > self.delay_bank[8] {
                self.countE = 0;
            }
            self.countF += 1;
            if self.countF > self.delay_bank[9] {
                self.countF = 0;
            }
            self.countG += 1;
            if self.countG > self.delay_bank[10] {
                self.countG = 0;
            }
            self.countH += 1;
            if self.countH > self.delay_bank[11] {
                self.countH = 0;
            }

            let outEL = self.arr_l.a_e[self.countE
                - if self.countE > self.delay_bank[8] {
                    self.delay_bank[8]
                } else {
                    0
                }];
            let outFL = self.arr_l.a_f[self.countF
                - if self.countF > self.delay_bank[9] {
                    self.delay_bank[9]
                } else {
                    0
                }];
            let outGL = self.arr_l.a_g[self.countG
                - if self.countG > self.delay_bank[10] {
                    self.delay_bank[10]
                } else {
                    0
                }];
            let outHL = self.arr_l.a_h[self.countH
                - if self.countH > self.delay_bank[11] {
                    self.delay_bank[11]
                } else {
                    0
                }];
            let outER = self.arr_r.a_e[self.countE
                - if self.countE > self.delay_bank[8] {
                    self.delay_bank[8]
                } else {
                    0
                }];
            let outFR = self.arr_r.a_f[self.countF
                - if self.countF > self.delay_bank[9] {
                    self.delay_bank[9]
                } else {
                    0
                }];
            let outGR = self.arr_r.a_g[self.countG
                - if self.countG > self.delay_bank[10] {
                    self.delay_bank[10]
                } else {
                    0
                }];
            let outHR = self.arr_r.a_h[self.countH
                - if self.countH > self.delay_bank[11] {
                    self.delay_bank[11]
                } else {
                    0
