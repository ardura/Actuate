...
impl ChorusEnsemble {
    pub fn new(sample_rate: f32, range: f32, speed: f32, amount: f32) -> Self {
    ...
    pub fn process(&mut self, left_in: f32, right_in: f32) -> (f32, f32) {
        ...
        left_out -= (self.left_buffer[count] - self.left_buffer[count + 1])
            - (self.left_buffer[count + 1] - self.left_buffer[count + 2]) / 50.0;
        ...
        right_out -= (self.right_buffer[count] - self.right_buffer[count + 1])
            - (self.right_buffer[count + 1] - self.right_buffer[count + 2]) / 50.0;
